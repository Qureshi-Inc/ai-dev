...
export function implementPrompt(args: {
  ...
  const system = [
    ...
    "- Do NOT create or modify GitHub Actions workflow files (.github/workflows/*) unless the",
    "  issue EXPLICITLY requires CI/CD changes. Never add a build/deploy pipeline that doesn't",
    "  fit the project (e.g. no npm build/deploy for a static single-file HTML site).",
